...
class TransitionToWinLose: NSObject, UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        toVC.lineView.frame = CGRect(x: lineFrame.minX, y: lineFrame.minY-toView.frame.height, width: lineFrame.width, height: lineFrame.height)
        ...
        UIView.animateKeyframes(withDuration: self.animationDuration, delay: 0.6, options: [], animations: {
            ...
            UIView.addKeyframe(withRelativeStartTime: 0.15, relativeDuration: 0.55, animations: {
                toVC.lineView.frame = CGRect(x: lineFrame.minX, y: lineFrame.minX, width: lineFrame.width, height: lineFrame.height)
                toVC.letterE.frame = CGRect(x: eFrame.minX, y: eFrame.minY, width: eFrame.width, height: eFrame.height)
                toVC.letterN.frame = CGRect(x: nFrame.minX, y: nFrame.minY, width: nFrame.width, height: nFrame.height)
                toVC.letterD.frame = CGRect(x: dFrame.minX, y: dFrame.minY, width: dFrame.width, height: dFrame.height)
            })
